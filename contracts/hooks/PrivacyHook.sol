...
/**
 * @title PrivacyHook
 * @notice Privacy hook for ERC-8183 Agentic Commerce Protocol.
 *         Providers must submit encrypted envelopes (IPFS CID + ECDH-wrapped
 *         AES keys) instead of plaintext deliverables. Optionally requires a
 *         zero-knowledge proof for each submission.
 *
 * USE CASE
 * --------
 * An agent job involves sensitive data (PII, proprietary models, private
 * inference results). The client wants the payment flow to remain public and
 * trustless (handled by core escrow) while keeping the actual deliverable
 * private. The provider encrypts the deliverable with AES-256-GCM, wraps
 * the AES key per recipient via ECDH, stores the blob on IPFS, and submits
 * the CID on-chain. The hook validates the envelope structure and an
 * optional ZK proof that the encrypted data satisfies job requirements.
 *
 ...
 *  3. fund(jobId, expectedBudget, "") — normal funding, no hook logic needed.
 *  4. Provider encrypts deliverable off-chain, uploads to IPFS:
 *     - AES-256-GCM encrypt data → ciphertext blob
 *     - ECDH-wrap AES key for each recipient → wrappedKeys[]
 *     - Upload blob to IPFS → CID (bytes32)
 *     - Optionally generate ZK proof over (jobId, CID, …)
 ...
 *  6. complete / reject — normal flow. On reject, commitment is cleared.
 *
 ...
 * envelopes. It does NOT verify encryption correctness (that requires the
 * decryption key). The optional ZK proof provides cryptographic assurance
 * that the encrypted data satisfies application-specific constraints.
 * Payment flow is entirely unchanged — the hook is pure policy enforcement.
 */
contract PrivacyHook is BaseACPHook {
    ...
    constructor(address acpContract_) BaseACPHook(acpContract_) {
        if (acpContract_ == address(0)) revert InvalidACPAddress();
    }
    ...
    function _postSetBudget(uint256 jobId, uint256, bytes memory optParams) internal override {
    ...
    function _preSubmit(uint256 jobId, bytes32 deliverable, bytes memory optParams) internal override {
    ...
    function _postSubmit(uint256 jobId, bytes32, bytes memory optParams) internal override {
